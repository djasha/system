...
export const GET: APIRoute = async ({ props }) => {
  ...
  const rawPath = path.resolve(process.cwd(), `src/content/patterns/${entry.id}.doc.mdx`);
  ...
  const componentCollection = await getCollection('components');
  const componentMap = new Map(componentCollection.map((c) => [c.id, c.data]));
  const resolvedRelated = entry.data.related.map((slug: string) => ({
    slug,
    ...(componentMap.get(slug) ?? {}),
  }));
  const resolvedComponentsUsed = (entry.data.components_used ?? []).map((slug: string) => ({
    slug,
    ...(componentMap.get(slug) ?? {}),
  }));
    ...
    JSON.stringify(
      {
        ...
        components_used_resolved: resolvedComponentsUsed,
